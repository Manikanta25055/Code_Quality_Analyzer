"""
Phase 2: Feature Extraction from Python Code
Extracts 25+ features from code repositories using AST analysis.
l
...
class CodeAnalyzer:
    ...
    def _extract_naming_features(self):
        ...
        all_names = []
        bad_names = 0  # Single letter variables
    ...
    def _extract_maintainability_features(self):
        ...
        # Import count (module coupling indicator)
...
class RepoFeatureExtractor:
    ...
    def download_repo(self, repo_name, clone_url):
        ...
        try:
            ...
            zip_url = clone_url.replace('.git', '/archive/refs/heads/main.zip')
    ...
    def extract_python_files(self, zip_content, max_files=20):
        ...
        try:
            with zipfile.ZipFile(zip_content) as zf:
                for file_info in zf.filelist[:100]:  # Limit search
...
def main():
    """Main execution"""
    print("=" * 70)
    print("Phase 2: Feature Extraction from Repositories")
    print("=" * 70)

    # Load repository list
    labels_df = pd.read_csv('../data/labels.csv')
    print(f"\nLoaded {len(labels_df)} repositories")

    # Initialize extractor
    extractor = RepoFeatureExtractor()

    # Extract features from ALL repos
    print(f"\nStarting feature extraction on all {len(labels_df)} repositories...")
    ...
    for idx, row in labels_df.iterrows():
        features = extractor.extract_repo_features(row['repo_name'], row['clone_url'])

        if features:
            features['repo_name'] = row['repo_name']
            features['quality_label'] = row['quality_label']
            results.append(features)
            success_count += 1
        else:
            fail_count += 1

        # Progress update every 20 repos
        if (idx + 1) % 20 == 0:
            print(f"\nProgress: {idx + 1}/{len(labels_df)} repos processed")
    ...
    if results:
        ...
        print("=" * 70)
        print(f"\nTotal repositories processed: {len(labels_df)}")
        print(f"Successfully extracted: {success_count} ({success_count/len(labels_df)*100:.1f}%)")
        print(f"Failed: {fail_count} ({fail_count/len(labels_df)*100:.1f}%)")
        print(f"Total features per repo: {len(results_df.columns) - 2}")
        print(f"Saved to: {output_file}")

        print("\nSample statistics:")
        print(results_df.describe().iloc[:, :5])

        print("\nNext Steps:")
        print("1. Check data/extracted_features.csv")
        print("2. Verify feature quality and distributions")
        print("3. Move to Phase 3: Data Preparation")
    else:
        print("\nNo features extracted. Check repository URLs.")
